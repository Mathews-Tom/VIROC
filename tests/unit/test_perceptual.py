...
def _square_frame(
    value: int = 255, *, x0: int = 8, y0: int = 8, size: int = 8
) -> tuple[tuple[int, ...], ...]:
    return tuple(
        tuple(value if x0 <= x < x0 + size and y0 <= y < y0 + size else 0 for x in range(32))
        for y in range(32)
    )
...
def test_phash_comparator_passes_near_identical_frame_sets() -> None:
    baseline = [_square_frame(), _square_frame()]
    candidate = [_square_frame(x0=9), _square_frame(x0=9)]

    comparison = compare_frame_sets(candidate, baseline, threshold=4)

    assert comparison.passed
    assert comparison.distances
    assert 0 < max(comparison.distances) <= 4
...
def test_phash_comparator_fails_just_beyond_threshold() -> None:
    baseline = perceptual_hash_frames([_square_frame()])
    candidate = perceptual_hash_frames([_square_frame(x0=9)])

    comparison = compare_perceptual_hashes(candidate, baseline, threshold=3)

    assert not comparison.passed
    assert comparison.distances == (4,)
...
def test_validate_perceptual_hash_returns_no_diagnostics_within_threshold() -> None:
    baseline = perceptual_hash_frames([_square_frame()])
    candidate = perceptual_hash_frames([_square_frame(x0=9)])

    assert validate_perceptual_hash(candidate, baseline, threshold=4) == []


def test_validate_perceptual_hash_reports_frame_count_mismatch() -> None:
    baseline = perceptual_hash_frames([_square_frame(), _square_frame()])
    candidate = perceptual_hash_frames([_square_frame()])

    diagnostics = validate_perceptual_hash(candidate, baseline, threshold=4)

    assert [diagnostic.code for diagnostic in diagnostics] == [VIR_PERCEPTUAL_MISMATCH]
    assert diagnostics[0].help == "per-frame Hamming distances: frame count mismatch; threshold: 4"


def test_compare_perceptual_hashes_rejects_negative_threshold() -> None:
    with pytest.raises(ValueError, match="threshold must be non-negative"):
        compare_perceptual_hashes("phash:0000000000000000", "phash:0000000000000000", threshold=-1)
