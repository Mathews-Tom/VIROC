...
from viroc.adapters.motion_canvas.emit import materialize_source
...
VIR_MISSING_NODE = code(DiagnosticClass.RENDERER, 61)
VIR_MISSING_NPX = code(DiagnosticClass.RENDERER, 62)
VIR_MISSING_MOTION_CANVAS = code(DiagnosticClass.RENDERER, 63)
VIR_MISSING_FFMPEG = code(DiagnosticClass.RENDERER, 64)
VIR_MISSING_FFPROBE = code(DiagnosticClass.RENDERER, 65)
VIR_TOOL_PROBE_FAILED = code(DiagnosticClass.RENDERER, 66)

_DEFAULT_OUTPUT_NAME = "viroc"
_DEFAULT_OUTPUT_DIR = "output"
_DEFAULT_TIMEOUT_SECONDS = 180
_DEFAULT_MOTION_CANVAS_COMMAND = ("npx", "--no-install", "motion-canvas")
...
class RenderEnvironmentError(RuntimeError):
    ...
    def __init__(self, diagnostics: list[Diagnostic]) -> None:
        super().__init__("Motion Canvas render prerequisites are unavailable")
        self.diagnostics = diagnostics


class RenderCommandError(RuntimeError):
    """Raised when the Motion Canvas CLI or FFmpeg exits unsuccessfully."""

    def __init__(self, command: Sequence[str], stderr: str) -> None:
        super().__init__(stderr or "render command failed")
...
def render(
    ...
    *,
    ...
    """Render a generated Motion Canvas project and mux captions with FFmpeg."""
    ...
    render_dir = out_dir / "motion-canvas-render"
...
def motion_canvas_version(ctx: BuildContext) -> str:
    """Return the installed Motion Canvas CLI version string."""
    output = _run_command([* _motion_canvas_command(ctx), "--version"], timeout=_timeout(ctx)).stdout.strip()
...
def _materialize_source(source: BuildArtifact, destination: Path) -> Path:
    ...
    if source.path is not None:
        ...
        return destination
    raise ValueError("source artifact did not carry a Motion Canvas project tree")


def _run_motion_canvas(project_dir: Path, ctx: BuildContext) -> Path:
    output_dir = project_dir / _renderer_str(ctx, "output_dir", _DEFAULT_OUTPUT_DIR)
    if output_dir.exists():
        shutil.rmtree(output_dir)
    command = [*_motion_canvas_command(ctx), "render"]
    ...
    candidates = sorted(output_dir.rglob("*.mp4")) if output_dir.exists() else []
    if not candidates:
        raise RenderCommandError(command, "motion-canvas render completed but produced no .mp4 output")
    return candidates[0]
...
def _write_build_manifest(source: BuildArtifact, video_path: Path, ctx: BuildContext) -> None:
    ...
    manifest = build_manifest(
        ...
        renderer_id="motion_canvas",
        renderer_version=motion_canvas_version(ctx),
...
def _motion_canvas_command(ctx: BuildContext) -> list[str]:
    configured = ctx.renderer.get("motion_canvas_command")
    if configured is None:
        return list(_DEFAULT_MOTION_CANVAS_COMMAND)
    ...
    raise ValueError("renderer.motion_canvas_command must be a string or sequence of strings")
...
def _output_name(ctx: BuildContext) -> str:
    return _renderer_str(ctx, "output_name", _DEFAULT_OUTPUT_NAME)
